...
const One = props => {
  ...
  const updateTwoClicks = () => {
    dispatchTwo(updateTwoScreenClicks(twoScreenClicks));
  }

  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      <Text style={{ fontSize: 40 }}>Screen One</Text>
      <Text style={{ fontSize: 40 }}>Clicks {oneScreenClicks}</Text>
      <Button title="Click" onPress={updateOneClicks} />
      <Text style={{ fontSize: 40 }}>Clicks {twoScreenClicks}</Text>
      <Button title="Click to update screen two" onPress={updateTwoClicks} />
    </View>
  );
};
